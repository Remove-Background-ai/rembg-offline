...
self.onmessage = async (evt) => {
    ...
    try {
        ...
        const previewBlob = await pCanvas.convertToBlob({ type: 'image/png', quality: 0.6 });
        // Attempt to close bitmap
        try {
            bitmap?.close?.();
        }
        catch { }
        const msg = { type: 'result', mainBlob, previewBlob };
        self.postMessage(msg);
    }
    catch (err) {
        const msg = { type: 'error', message: err?.message || String(err) };
        self.postMessage(msg);
    }
};
export {};
